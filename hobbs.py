# Modified from https://github.com/cmward/hobbs/blob/master/hobbs.py Chris Ward, 2014
...
def find_pronouns(tree):
    """ Traverse the tree and returns the pronouns locations """
    ...
    while not queue.empty():
        node = queue.get()
        # if the node is an PRP, add it to PRP
        if "PRP" in node.label():
            PRP.append(get_pos(tree, node))
        for child in node:
            if isinstance(child, nltk.Tree):
                queue.put(child)
    return PRP
